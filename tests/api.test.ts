...
describe('Tests for endpoints', () => {
  ...
  test('POST - /api/washingMachine - empty body', async (done: DoneCallback): Promise<void> => {
  ...
  test('POST - /api/washingMachine - with data', async (done: DoneCallback): Promise<void> => {
    const response: supertest.Response = await request(app).post('/api/washingMachine').send({
      serialNumber: 936,
      model: 'testing model',
      dateOfManufacture: Date.now().toLocaleString(),
    });
  ...
  test('GET - /api/washingMachine - get all machines', async (done: DoneCallback): Promise<void> => {
    const response: supertest.Response = await request(app).get('/api/washingMachine');
  ...
  test('GET - /api/washingMachine/:serialNumber - get machine with serial number 388',
      async (done: DoneCallback): Promise<void> => {
        const response: supertest.Response = await request(app).get('/api/washingMachine/388');
        const {status, data}: { status: string, data: IWashingMachine } = response.body;
        expect(status).toBe('success');
        expect(data).toBeInstanceOf(Object);
  ...
  test('GET - /api/washingMachine/:serialNumber - get error if serial number isn\'t exist',
      async (done: DoneCallback): Promise<void> => {
        const response: supertest.Response = await request(app).get('/api/washingMachine/0');
        const {status, data}: { status: string, data: string } = response.body;
  ...
  test('GET - /api/washingMachine/model - get machines with the same model',
      async (done: DoneCallback): Promise<void> => {
        const response: supertest.Response = await request(app).get('/api/washingMachine/model').send({
          model: 'testing model',
        });
        const {status, data}: { status: string, data: Array<IWashingMachine> } = response.body;
  ...
  test('GET - /api/washingMachine/model - get error if model isn\'t exist',
      async (done: DoneCallback): Promise<void> => {
        const response: supertest.Response = await request(app).get('/api/washingMachine/model').send({
  ...
  test('GET - /api/washingMachine/status - get machines with the same status',
      async (done: DoneCallback): Promise<void> => {
        const response: supertest.Response = await request(app).get('/api/washingMachine/status').send({
          status: true,
        });
        const {status, data}: { status: string, data: Array<IWashingMachine> } = response.body;
        expect(response.status).toBe(200);
  ...
  test('GET - /api/washingMachine/status - get an empty array if status isn\'t exist',
      async (done: DoneCallback): Promise<void> => {
        const response: supertest.Response = await request(app).get('/api/washingMachine/status').send({
          status: false,
        });
